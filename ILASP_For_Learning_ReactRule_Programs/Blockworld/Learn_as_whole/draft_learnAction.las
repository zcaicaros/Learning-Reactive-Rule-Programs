...
rule(1,T,m):- ruleapp(T,1,m),not higher_rule_m(T,1).
rule(2,T,m):- ruleapp(T,2,m),not higher_rule_m(T,2).
rule(3,T,m):- ruleapp(T,3,m),not higher_rule_m(T,3).
rule(4,T,m):- ruleapp(T,4,m),not higher_rule_m(T,4).
rule(5,T,m):- ruleapp(T,5,m),not higher_rule_m(T,5).
rule(6,T,m):- ruleapp(T,6,m),not higher_rule_m(T,6).
rule(7,T,m):- ruleapp(T,7,m),not higher_rule_m(T,7).
rule(8,T,m):- ruleapp(T,8,m),not higher_rule_m(T,8).
rule(9,T,m):- ruleapp(T,9,m),not higher_rule_m(T,9).
rule(T,B,1,s):- ruleapp(T,B,1,s),not higher_rule_s(T,1).
rule(T,AB,2,s):- ruleapp(T,AB,2,s),not higher_rule_s(T,2).
rule(T,AB,3,s):- ruleapp(T,AB,3,s),not higher_rule_s(T,3).
...
#modeh(already_clear(var(block),var(time)),(positive)).
#modeh(move_on(var(block),t,var(time)),(positive)).
#modeh(make_clear_h(var(block),var(time)),(positive)).
#modeb(1,rule(const(priority),var(time),m)).
#modeb(1,rule(var(time),var(block),const(cp),s)).
